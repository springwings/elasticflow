...
public final class KafkaSearcher extends SearcherFlowSocket {
	...
	public SearcherResult Search(SearcherModel<?, ?> searcherModel, String instance, SearcherHandler handler)
        ...
        props.put("bootstrap.servers", this.connectParams.getWhp().getHost());		        
		...
		try { 
			...
			conf.put(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, this.connectParams.getWhp().getHost());
			conf.put(AdminClientConfig.REQUEST_TIMEOUT_MS_CONFIG, "5000");
	        AdminClient adminClient = AdminClient.create(conf);
	        DescribeTopicsResult topicResult = adminClient.describeTopics(Arrays.asList(this.connectParams.getWhp().getDefaultValue().getString("consumer.topic").split(",")));
