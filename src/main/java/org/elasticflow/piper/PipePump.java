...
public final class PipePump extends Instruction {
	...
	private void singleThread(String instance,String mainName,String L1seq,String L2seq,JOB_TYPE job_type,String storeId,String originalSql,ConcurrentLinkedDeque<String> pageList,HashMap<String, String> param,SQLParam scanParam,String writeTo,AtomicInteger total) throws FNException { 
		...
		while(!pageList.isEmpty()){
			...
			if ((this.readHandler==null || !this.readHandler.loopScan(param)) && rState.getReaderScanStamp().compareTo(GlobalParam.SCAN_POSITION.get(instance).getL2SeqPos(L2seq)) > 0) {
	...
	public class Pump implements Runnable { 
		...
		public void run() {
			...
			while(!pageList.isEmpty()){
				...
				if ((readHandler==null || !readHandler.loopScan(param)) && rState.getReaderScanStamp().compareTo(GlobalParam.SCAN_POSITION.get(instance).getL2SeqPos(L2seq)) > 0) {
					GlobalParam.SCAN_POSITION.get(instance).updateL2SeqPos(L2seq, rState.getReaderScanStamp());
				}
				if (job_type==JOB_TYPE.INCREMENT) { 
					Common.saveTaskInfo(instance, L1seq,storeId,GlobalParam.JOB_INCREMENTINFO_PATH);
